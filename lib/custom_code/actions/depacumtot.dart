...
Future depacumtot(BuildContext context) async {
  ...
  final result = await showDialog<Map<String, dynamic>>(
    ...
    builder: (context) {
      ...
      // Mínimo permitido ahora es 2024
  ...
  try {
    ...
    // ---------------------------------------------------------------------------
    // PASO B: Depreciación ACUMULADA — suma TODOS los registros con
    // aniodepreciacion <= anioSeleccionado (sin importar cuán atrás estén).
    // ---------------------------------------------------------------------------
    Query queryDep = firestore.collection('calculodepreciacion');
    // *** CAMBIO CLAVE: en lugar de isEqualTo usamos isLessThanOrEqualTo ***
    queryDep = queryDep.where('aniodepreciacion',
        isLessThanOrEqualTo: anioSeleccionado);
    if (filtroUnidad != 'TODOS') {
          ...
          children: [
            ...
            pw.Column(
              ...
              children: [
                ...
                // *** CAMBIO: etiqueta indica acumulado HASTA el año ***
    ...
    // Header Tabla — columna ahora dice "Depreciación Acumulada"
    filasTabla.add(
      pw.TableRow(
        ...
        build: (context) => [
          pw.Table(
            ...
            columnWidths: {
              ...
              2: const pw.FlexColumnWidth(3), // Depreciacion acumulada
            },
            children: filasTabla,
          ),
        ],
