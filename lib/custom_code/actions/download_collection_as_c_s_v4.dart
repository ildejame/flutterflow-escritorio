...
Future downloadCollectionAsCSV4(List<EstablecimientosRecord>? promo) async {
  promo = promo ?? [];
  ...
  String contenidoArchivo =
      "NombreEstablecimiento,NombrePropietario,Movimiento,Giro,FotoLocal,Fecha,Posicion,CodigoQR";

  promo.forEach((record) {
    contenidoArchivo += "\n${limpiarTexto(record.nombreEstablecimiento)},"
        "${limpiarTexto(record.nombrePropietario)},"
        "${limpiarTexto(record.movimiento)},"
        "${limpiarTexto(record.giroDeColeccion)},"
        "${limpiarTexto(record.fotoLocal)},"
        "${formatearFecha(record.fecha)},"
        "${formatearCoordenadas(record.posicion?.latitude, record.posicion?.longitude)},"
        "${limpiarTexto(record.codigoQR)}";
  });

  final nombreArchivo = "Establecimientos${DateTime.now()}.csv";
