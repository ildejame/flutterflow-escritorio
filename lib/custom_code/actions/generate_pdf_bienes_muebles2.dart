...
Future<void> generatePdfBienesMuebles2(
  ...
  page.graphics.drawImage(logo, Rect.fromLTWH(30, 10, 80, 60));
  ...
  // Sección de encabezado a la derecha del logo
  final double headerX = 120;
  final double headerWidth = pageSize.width - headerX - 30;
  ...
  page.graphics.drawString(
    ...
    font,
    bounds: Rect.fromLTWH(headerX, 35, headerWidth, 60),
    format: PdfStringFormat(alignment: PdfTextAlignment.center),
  );

  // Fecha y ubicación
  final double footerY = 100;
  page.graphics.drawString(
    "FECHA:",
    boldFont,
    bounds: Rect.fromLTWH(pageSize.width - 200, footerY, 50, 15),
    format: PdfStringFormat(alignment: PdfTextAlignment.left),
  );
  page.graphics.drawString(
    "05 DE DICIEMBRE DE 2024",
    font,
    bounds: Rect.fromLTWH(pageSize.width - 150, footerY, 150, 15),
    format: PdfStringFormat(alignment: PdfTextAlignment.left),
  );

  // Ubicación y datos específicos
  double currentY = 110;
  page.graphics.drawString(
    "UBICACIÓN: JUZGADO DE PROCESO Y PROCEDIMIENTO PENAL ORAL",
    boldFont,
    bounds: Rect.fromLTWH(30, currentY, pageSize.width - 60, 12),
    format: PdfStringFormat(alignment: PdfTextAlignment.left),
  );
  currentY += 15;
  page.graphics.drawString(
  ...
  // Dibujar firmas
  currentY += grid.rows.count * 20 + 30;
  page.graphics.drawString(
    "DEPOSITARIOS",
    boldFont,
  ...
  // Dibujar la leyenda final
  currentY += 80;
  final Rect legendBounds =
      Rect.fromLTWH(50, currentY, pageSize.width - 100, 60);
  page.graphics
      .drawRectangle(bounds: legendBounds, pen: PdfPen(PdfColor(0, 0, 0)));
  page.graphics.drawString(
    ...
    font,
    bounds: Rect.fromLTWH(55, currentY + 10, pageSize.width - 110, 40),
    format: PdfStringFormat(alignment: PdfTextAlignment.left),
  );
